...
public class DiscordSRV extends JavaPlugin {
    ...
    public void init() {
        ...
        if (serverIsLog4jCapable) {
            ...
            consoleAppender = new ChannelLoggingHandler(() -> {
                ...
                return textChannel != null && textChannel.getGuild().getSelfMember().hasPermission(textChannel,
                        Permission.MESSAGE_READ, Permission.MESSAGE_WRITE) ? textChannel : null;
            }, config -> {
                config.setLoggerNamePadding(config().getInt("DiscordConsoleChannelPadding"));
                config.setLogLevels(EnumSet.copyOf(config().getStringList("DiscordConsoleChannelLevels").stream()
                        .map(String::toUpperCase).map(LogLevel::valueOf).collect(Collectors.toSet())));
    ...
    public void reloadCancellationDetector() {
        ...
        if (Debug.MINECRAFT_TO_DISCORD.isVisible()) {
            try {
                legacyCancellationDetector = new CancellationDetector<>(this, AsyncPlayerChatEvent.class,
                        (listener, event) -> {
    ...
    public void processChatMessage(Player player, Component message, String channel, boolean cancelled) {
        ...
        if (MessageUtil.strip(legacy).startsWith(prefix) == blacklist) {
            debug(Debug.MINECRAFT_TO_DISCORD,
                    "User " + player.getName() + " sent a message but it was not delivered to Discord because "
                            + (blacklist ? "the message started with \"" + prefix
                                    : "the message didn't start with \"" + prefix)
    ...
    public void broadcastMessageToMinecraftServer(String channel, String message, User author) {
        ...
        if (authorLinkedUuids != null) {
            ...
            if (authorLinkedUuids.isLeft()) {
                uuid = authorLinkedUuids.left().get();
            } else if (authorLinkedUuids.isRight()) {
                uuid = authorLinkedUuids.right().get().javaID;
            } else {
                return;
            }
    ...
    public boolean isGroupRoleSynchronizationEnabled(boolean checkPermissions) {
        ...
        for (Map.Entry<String, String> entry : groupsAndRolesToSync.entrySet()) {
            ...
            if (!group.isEmpty()) {
                ...
                if (!(roleId.isEmpty() || roleId.replace("0", "").trim().isEmpty()))
                    return true;
            }
        }
        return false;

...
public class DiscordAccountLinkListener extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        String reply = DiscordSRV.getPlugin().getAccountLinkManager().process(receivedMessage.getContentRaw(),
                event.getAuthor().getId());
        if (reply != null) {
            receivedMessage.reply(reply).queue(replyMessage -> {
                // delete the message after a delay if the config option is enabled
                int deleteSeconds = DiscordSRV.config().getIntElse("MinecraftDiscordAccountLinkedMessageDeleteSeconds",
                        0);
                if (deleteSeconds > 0) {
        ...
        if (uuid != null) {
            ...
            if (roleToAdd == null || roleToAdd.getGuild().equals(member.getGuild())) {
                ...
                DiscordSRV.debug(Debug.GROUP_SYNC,
                        "Not adding role to member upon guild join due to the guild being different! ("
                                + roleToAdd.getGuild() + " / " + member.getGuild() + ")");
            }

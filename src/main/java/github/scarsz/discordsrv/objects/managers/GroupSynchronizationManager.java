...
public class GroupSynchronizationManager extends ListenerAdapter implements Listener {
    ...
    private final Map<UUID, Map<String, List<String>>> justModifiedGroups = new ExpiringDualHashBidiMap<>(
            ...
            DiscordUtil.getJda().getGuilds().stream().flatMap(guild -> guild.getMembers().stream())
                    ...
                    .filter(data -> (Objects.nonNull(data) && !data.isEmpty())).forEach(data -> {
                        if (data.isLeft()) {
                            ...
                            playerList.add(Bukkit.getOfflinePlayer(user.javaID));
                            playerList.add(Bukkit.getOfflinePlayer(user.bedrockID));
                        }
                    });
    ...
    public void resync(User user, SyncDirection direction, SyncCause cause) {
        ...
        if (data.isLeft()) {
            ...
            if (uuid == null) {
                DiscordSRV.debug(Debug.GROUP_SYNC, "Tried to sync groups for " + user
                        + " but their Discord account is not linked to a MC account");
                return;
            }
            resync(Bukkit.getOfflinePlayer(uuid), direction, cause);
        } else if (data.isRight()) {
            JBUser jbUser = data.right().get();
            if (jbUser.javaID == null && jbUser.bedrockID == null) {
                DiscordSRV.debug(Debug.GROUP_SYNC, "Tried to sync groups for " + user
                        + " but their Discord account is not linked to a MC account");
                return;
            }
            resync(Bukkit.getOfflinePlayer(jbUser.javaID), direction, cause);
            resync(Bukkit.getOfflinePlayer(jbUser.bedrockID), direction, cause);
        }
    }
    ...
    public void resync(OfflinePlayer player, SyncDirection direction, boolean addLinkedRole, SyncCause cause) {
        if (player == null)
            return;
        if (getPermissions() == null) {
            DiscordSRV.debug(Debug.GROUP_SYNC,
                    "Can't synchronize groups/roles for " + player.getName() + ", permissions provider is null");
            return;
        }
        ...
        if (DiscordSRV.getPlugin().getAccountLinkManager() == null) {
            DiscordSRV.debug(Debug.GROUP_SYNC, "Tried to sync groups for player " + player.getName()
                    + " but the AccountLinkManager wasn't initialized yet");
            return;
        }
        ...
        if (discordId == null) {
            DiscordSRV.debug(Debug.GROUP_SYNC, "Tried to sync groups for player " + player.getName()
                    + " but their MC account is not linked to a Discord account. Synchronization cause: " + cause);
            return;
        }
        User user = DiscordUtil.getUserById(discordId);
        if (user == null) {
            DiscordSRV.debug(Debug.GROUP_SYNC,
                    "Tried to sync groups for player " + player.getName() + " but Discord user is not available");
            return;
        }
        ...
        synchronizationSummary.add("Group synchronization (#" + id + ") " + direction + " for " + "{" + player.getName()
                + ":" + user + "}. Synchronization cause: " + cause);
        ...
        boolean minecraftIsStrictlyAuthoritative = oneWaySynchronisation
                && DiscordSRV.config().getBoolean("GroupRoleSynchronizationMinecraftIsAuthoritative");
        boolean discordIsStrictlyAuthoritative = oneWaySynchronisation
                && !DiscordSRV.config().getBoolean("GroupRoleSynchronizationMinecraftIsAuthoritative");
        if (oneWaySynchronisation)
            synchronizationSummary.add("Synchronisation is one way ("
                    + (minecraftIsStrictlyAuthoritative ? "Minecraft -> Discord" : "Discord -> Minecraft") + ")");

        if ((minecraftIsStrictlyAuthoritative && direction == SyncDirection.TO_MINECRAFT)
                || (discordIsStrictlyAuthoritative && direction == SyncDirection.TO_DISCORD)) {
            DiscordSRV.debug(Debug.GROUP_SYNC,
                    "Group synchronization (#" + id + ") " + direction + " cancelled because "
                            + (minecraftIsStrictlyAuthoritative ? "Minecraft" : "Discord")
        ...
        for (Map.Entry<String, String> entry : DiscordSRV.getPlugin().getGroupSynchronizables().entrySet()) {
            ...
            if (StringUtils.isBlank(groupName))
                continue;
            if (StringUtils.isBlank(roleId.replace("0", "")))
                continue;
            ...
            if (membersNotInGuild.contains(user.getId())) {
                synchronizationSummary.add("Tried to sync role " + role
                        + " but the user wasn't a member in the guild the role is in (cached)");
                continue;
            }
            ...
            try {
                ...
                if (e.getErrorResponse() == ErrorResponse.UNKNOWN_MEMBER) {
                    ...
                    synchronizationSummary.add("Tried to sync role " + role
                            + " but the user wasn't a member in the guild the role is in (Discord response)");
                    continue;
                }
                DiscordSRV.error(e);
                continue;
            ...
            if (member == null) {
                ...
                synchronizationSummary.add("Tried to sync " + role + " but could not find " + user
                        + " in the role's Discord server, treating it as if they don't have the role");
            }
            ...
            try {
                ...
                if (playerGroups == null) {
                    synchronizationSummary.add("Tried to sync {" + role + ":" + groupName
                            + "} but Vault returned null as the player's groups (Player is "
            ...
            if (!vaultGroupsLogged) {
                synchronizationSummary.add("Player " + player.getName() + "'s "
                        + (primaryGroupOnly ? "Primary group: " + getPermissions().getPrimaryGroup(null, player) + ", "
                                : "")
                        + "Vault groups: " + Arrays.toString(playerGroups) + " (Player is "
                        + (player.isOnline() ? "online" : "offline") + ")");
            ...
            boolean hasGroup;
            try {
                hasGroup = primaryGroupOnly ? groupName.equalsIgnoreCase(getPermissions().getPrimaryGroup(null, player))
            ...
            // Managed roles cannot be given or taken, so it will be Discord -> Minecraft
            // only
            if (roleIsManaged && minecraftIsStrictlyAuthoritative) {
                synchronizationSummary.add("Tried to sync {" + role + ":" + groupName
                        + "} to Discord but the role is managed and Minecraft is strictly authoritative");
                continue;
            }
            ...
            if (hasGroup == hasRole) {
                ...
                if (minecraftIsAuthoritative) {
                        ...
                        try {
                            ...
                            if (ArrayUtils.contains(serverGroups, groupName)) {
                                if (!getPermissions().playerAddGroup(null, player, groupName)) {
                                    DiscordSRV.debug(Debug.GROUP_SYNC,
                                            "Synchronization #" + id + " for {" + player.getName() + ":" + user
                                                    + "} failed: adding group " + groupName + ", returned a failure");
                                    additions.remove(groupName);
                                }
                            } else {
                                DiscordSRV.debug(Debug.GROUP_SYNC,
                                        "Synchronization #" + id + " for {" + player.getName() + ":" + user
                                                + "} failed: group " + groupName + " doesn't exist (Server's Groups: "
                                                + Arrays.toString(serverGroups) + ")");
                            }
                        } catch (Throwable t) {
                    ...
                    synchronizationSummary.add("{" + groupName + ":" + role + "} adds Minecraft group"
                            + (roleIsManaged ? " (Managed Role)" : ""));
                }
            } else { // hasGroup && !hasRole
                if (minecraftIsAuthoritative) {
                    roleChanges.computeIfAbsent(role.getGuild(), g -> new HashMap<>())
                            .computeIfAbsent("add", s -> new HashSet<>()).add(role);
                    synchronizationSummary.add("{" + groupName + ":" + role + "} adds Discord role");
                } else {
                    boolean luckPerms = PluginUtil.pluginHookIsEnabled("LuckPerms");
                    List<String> removals = justModifiedGroups
                            .computeIfAbsent(player.getUniqueId(), key -> new HashMap<>())
                            .computeIfAbsent("remove", key -> new ArrayList<>());
                    Runnable runnable = () -> {
                        try {
                            if (getPermissions().playerInGroup(null, player, groupName)) {
                                if (!getPermissions().playerRemoveGroup(null, player, groupName)) {
                                    DiscordSRV.debug(Debug.GROUP_SYNC,
                                            "Synchronization #" + id + " for {" + player.getName() + ":" + user
                                                    + "} failed: removing group " + groupName + " returned a failure");
                                    removals.add(groupName);
                                }
                            } else {
                                DiscordSRV.debug(Debug.GROUP_SYNC,
                                        "Synchronization #" + id + " for {" + player.getName() + ":" + user
                                                + "} failed: player is not in group \"" + groupName + "\"");
                                removals.add(groupName);
                            }
                        } catch (Throwable t) {
        ...
        if (!groupsDeniedByPermission.isEmpty()) {
            synchronizationSummary.add("The player does not have the following groups due to having the "
                    + "discordsrv.sync.deny.<group name> permission(s): "
                    + String.join(" | ", groupsDeniedByPermission));
        }
        ...
        if (addLinkedRole) {
            try {
                ...
                if (role != null) {
                    ...
                    DiscordSRV.debug(Debug.GROUP_SYNC,
                            "Couldn't add user to null (\"linked\") role to " + player.getName());
                }
            } catch (Throwable t) {
                DiscordSRV.debug(Debug.GROUP_SYNC, "Couldn't add \"linked\" role to " + player.getName()
                        + " due to exception: " + ExceptionUtils.getMessage(t));
            }
        }
        ...
        for (Map.Entry<Guild, Map<String, Set<Role>>> guildEntry : roleChanges.entrySet()) {
            ...
            if (member == null) {
                synchronizationSummary
                        .add("Synchronization failed for " + user + " in " + guild + ": user is not a member");
                continue;
            }
            ...
            if (!selfMember.canInteract(member)) {
                synchronizationSummary.add("Synchronization failed for " + member + ": can't interact with member"
                        + (member.isOwner() ? " (server owner)"
                                : !member.getRoles().isEmpty() ? !selfMember.getRoles().isEmpty()
                                        ? selfMember.getRoles().get(0).getPosition() <= member.getRoles().get(0)
                                                .getPosition()
                                                        ? " (member has a higher or equal role: "
                                                                + member.getRoles().get(0) + " ("
                                                                + member.getRoles().get(0).getPosition() + "))"
                                                        : " (bot has a higher role????? bot: "
                                                                + selfMember.getRoles().get(0) + ", member: "
                                                                + member.getRoles().get(0) + ")"
                                        : " (bot has 0 roles)" : " (bot & member both have 0 roles)"));
                synchronizationSummary.add("Bot's top role in " + guild + ": " + (selfMember.getRoles().isEmpty()
                        ? "bot has no roles"
            ...
            if (!selfMember.hasPermission(net.dv8tion.jda.api.Permission.MANAGE_ROLES)) {
                synchronizationSummary
                        .add("Synchronization failed for " + member + ": bot doesn't have MANAGE_ROLES permission");
                continue;
            }
            ...
            while (addIterator.hasNext()) {
                ...
                if (!selfMember.canInteract(role)) {
                    synchronizationSummary.add("Synchronization for role " + role + " (add) in " + guild
                            + " failed: can't interact with role (" + role.getPosition() + ")");
                    addIterator.remove();
            ...
            while (removeIterator.hasNext()) {
                ...
                if (!selfMember.canInteract(role)) {
                    synchronizationSummary.add("Synchronization for role " + role + " (remove) in " + guild
                            + " failed: can't interact with role (" + role.getPosition() + ")");
            ...
            guild.modifyMemberRoles(member, add, remove).reason("DiscordSRV synchronization").queue(
                    v -> DiscordSRV.debug(Debug.GROUP_SYNC,
                            "Synchronization #" + id + " for {" + player.getName() + ":" + member + "} successful in "
                                    + guild + ": {add=" + add + ", remove=" + remove + "}"),
                    t -> DiscordSRV.debug(Debug.GROUP_SYNC, "Synchronization #" + id + " for {" + player.getName() + ":"
                            + member + "} failed in " + guild + ": " + ExceptionUtils.getStackTrace(t)));
    ...
    public void resyncEveryone(SyncDirection direction, SyncCause cause) {
        ...
        DiscordSRV.getPlugin().getAccountLinkManager()
                .getManyDiscordIds(Arrays.stream(Bukkit.getOfflinePlayers()).map(OfflinePlayer::getUniqueId)
                        .collect(Collectors.toSet()))
                .keySet().stream().map(Bukkit::getOfflinePlayer).filter(Objects::nonNull).forEach(players::add);

        // synchronize everyone with a linked account in the connected discord servers
        Map<String, Either<UUID, JBUser>> linkedDiscords = DiscordSRV.getPlugin().getAccountLinkManager()
                .getManyUuids(DiscordUtil.getJda().getGuilds().stream().flatMap(guild -> guild.getMembers().stream())
                        .map(ISnowflake::getId).collect(Collectors.toSet()));
        Queue<OfflinePlayer> playerList = new LinkedList<>();
        DiscordUtil.getJda().getGuilds().stream().flatMap(guild -> guild.getMembers().stream())
    ...
    public void removeSynchronizables(OfflinePlayer player) {
        if (!DiscordSRV.config().getBoolean("GroupRoleSynchronizationMinecraftIsAuthoritative")) {
            ...
            DiscordSRV.debug(Debug.GROUP_SYNC,
                    player.getName() + " removed from their groups (Discord is authoritative). succeeded: " + success
                            + ", failed: " + fail);
        } else {
    ...
    public void removeSynchronizedRoles(OfflinePlayer player) {
        ...
        if (user != null) {
            ...
            DiscordSRV.getPlugin().getGroupSynchronizables().values().stream().map(DiscordUtil::getRole)
            ...
            try {
                ...
                DiscordSRV.debug(Debug.GROUP_SYNC, "Failed to remove \"linked\" role from " + player.getName()
                        + " during unlink: " + ExceptionUtils.getMessage(t));
            }
    ...
    public void onGuildMemberJoin(@NotNull GuildMemberJoinEvent event) {
        membersNotInGuilds.computeIfAbsent(event.getGuild().getId(), key -> new HashSet<>())
                .remove(event.getMember().getId());
    }
    ...
    private void onGuildMemberRolesChanged(String type, Member member, List<Role> roles) {
        ...
        if (checkRoles.isEmpty())
            return; // none of the changed roles were ones that would be synchronized
    ...
    private final List<Pattern> patterns = Arrays.asList(
            // GroupManager
            Pattern.compile("/?manu(?:add(?:sub)?|del(?:sub)?|promote|demote) " + userRegex + ".*",
                    Pattern.CASE_INSENSITIVE),
            // PermissionsEx
            Pattern.compile("/?pex user " + userRegex + " group(?: timed)? (?:add)|(?:set)|(?:remove) .*",
                    Pattern.CASE_INSENSITIVE),
            // zPermissions
    ...
    private void checkCommand(String message) {
        ...
        OfflinePlayer target = patterns.stream().map(pattern -> pattern.matcher(message)).filter(Matcher::find)
                .map(matcher -> matcher.group(1)).filter(Objects::nonNull).map(input -> {
        ...
        // run task later so that this command has time to execute & change the group
        // state
        Bukkit.getScheduler().runTaskLaterAsynchronously(DiscordSRV.getPlugin(),
                () -> resync(target, SyncDirection.TO_DISCORD, SyncCause.MINECRAFT_GROUP_EDIT_COMMAND), 5);
    }
    ...
    public Permission getPermissions() {
        if (permission != null) {
            ...
            try {
                ...
                RegisteredServiceProvider<Permission> provider = Bukkit.getServer().getServicesManager()
                ...
                if (!warnedAboutMissingVault && DiscordSRV.getPlugin().isGroupRoleSynchronizationEnabled(false)) {
                    DiscordSRV.error(
                            "Group synchronization failed: Vault classes couldn't be found (did it enable properly?). Vault is required for synchronization to work.");
    ...
    public enum SyncDirection {
        ...
        TO_MINECRAFT("to Minecraft"), TO_DISCORD("to Discord"), AUTHORITATIVE("on authority");
    ...
    public enum SyncCause {
        ...
        DISCORD_ROLE_EDIT("Discord roles changed"), MINECRAFT_GROUP_EDIT_API("Minecraft group change (via api)"),
        MINECRAFT_GROUP_EDIT_COMMAND("Minecraft group change (via command)"), PLAYER_LINK("Player linked"),
        PLAYER_JOIN("Player joined"), TIMER("Timer"), MANUAL("Manually triggered via resync command"),
